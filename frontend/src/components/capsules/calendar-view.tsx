  ...
  return (
    <section className="relative isolate min-h-[calc(100svh-var(--tc-shell-offset,4rem))] overflow-hidden bg-[#050816] px-4 py-8 lg:px-8">
      <div className="pointer-events-none absolute inset-0 -z-20" aria-hidden="true">
        <SpaceBackgroundFrame className="opacity-[0.15] blur-[1px]" restoreSnapshot startSettled />
      </div>
      <div className="pointer-events-none absolute inset-0 -z-10" aria-hidden="true">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_76%_30%,rgba(124,92,255,0.1)_0%,rgba(124,92,255,0)_44%)]" />
      </div>
      ...
      <div className="relative mx-auto max-w-5xl">
        ...
        <div className="rounded-2xl border border-white/14 bg-slate-950/58 shadow-[0_30px_80px_rgba(2,6,23,0.58)] backdrop-blur-xl">
          <div className="flex items-center justify-between border-b border-white/12 px-6 py-4">
            <Button
              variant="ghost"
              size="icon"
              onClick={() => setCurrentMonth(subMonths(currentMonth, 1))}
              className="text-slate-300 hover:bg-white/[0.08] hover:text-slate-100"
            >
            ...
            {loading ? (
              <div className="grid grid-cols-7 gap-1">
                {Array.from({ length: 35 }).map((_, index) => (
                  <Skeleton key={index} className="h-16 rounded-lg bg-white/[0.08]" />
                ))}
              </div>
            ) : (
              <div className="grid grid-cols-7 gap-1">
                  ...
                  return (
                    <button
                      ...
                      className={`relative flex min-h-[64px] cursor-pointer flex-col items-center gap-0.5 rounded-lg border p-1.5 text-sm transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/60 ${
                        ...
                        isSelected
                          ? "border-cyan-200/70 bg-cyan-300/20 shadow-[0_0_0_1px_rgba(94,230,255,0.4),0_10px_22px_rgba(8,21,46,0.46)]"
                          : "bg-transparent hover:-translate-y-[1px] hover:border-cyan-200/65 hover:bg-[radial-gradient(circle_at_50%_40%,rgba(94,230,255,0.2)_0%,rgba(94,230,255,0.08)_60%)] hover:shadow-[0_0_0_1px_rgba(94,230,255,0.45),0_10px_24px_rgba(5,12,30,0.56)]"
                      }`}
                    >
                      <span className={`text-xs font-medium ${today ? "text-cyan-200" : inMonth ? "text-slate-100" : "text-slate-500"}`}>
                        {format(day, "d")}
                      </span>
                ...
                <div className="flex flex-col gap-2">
                  {selectedCapsules.map((capsule) => (
                    <button
                      ...
                      className="flex cursor-pointer items-center gap-3 rounded-xl border border-white/12 bg-white/[0.04] p-3 text-left transition-colors hover:bg-white/[0.08]"
                    >
