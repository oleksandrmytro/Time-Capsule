...
export function CreateCapsuleForm({ onSubmit, onCancel, error: parentError }) {
  ...
  // Generate min/max dates for datetime-local inputs
  const minDate = new Date()
  minDate.setMinutes(minDate.getMinutes() + 1) // At least 1 minute in future
  ...
  async function handleSubmit(e) {
    ...
    const unlockAt = fd.get("unlockAt")
    const expiresAt = fd.get("expiresAt")

    if (!title) { setLocalError({ message: "Title is required" }); return }
    if (!unlockAt) { setLocalError({ message: "Unlock date is required" }); return }

    const unlockDate = new Date(unlockAt)
    const now = new Date()

    // Validate unlock date
    if (unlockDate <= now) { setLocalError({ message: "Unlock date must be in the future" }); return }
    if (unlockDate.getFullYear() > 2100) { setLocalError({ message: "Unlock year cannot exceed 2100" }); return }
    if (unlockDate.getFullYear() < now.getFullYear()) { setLocalError({ message: "Invalid unlock date" }); return }

    // Validate expires date if provided
    if (expiresAt) {
      const expiresDate = new Date(expiresAt)
      if (expiresDate.getFullYear() > 2100) { setLocalError({ message: "Expiry year cannot exceed 2100" }); return }
      if (expiresDate <= unlockDate) { setLocalError({ message: "Expiry date must be after unlock date" }); return }
    }
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-10 lg:px-8">
      <Button variant="ghost" size="sm" onClick={onCancel} className="mb-6 gap-1.5 text-muted-foreground -ml-3"><ArrowLeft className="h-4 w-4" /> Back</Button>
