...
export function HeroSection({ isAuthenticated, onGo, onLoadCapsules }) {
  return (
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <div className="mt-10 flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
            {isAuthenticated ? (
              <>
                <Button size="lg" className="h-12 px-8 text-base font-semibold" onClick={() => onGo('createCapsule')}>
                  Create Capsule <ArrowRight className="ml-2 h-4 w-4" />
                </Button>
                <Button variant="outline" size="lg" className="h-12 px-8 text-base font-semibold" onClick={async () => { await onLoadCapsules(); onGo('capsules') }}>
                  My Capsules
                </Button>
              </>
            ) : (
              <>
                <Button size="lg" className="h-12 px-8 text-base font-semibold" onClick={() => onGo('register')}>
                  Get Started Free <ArrowRight className="ml-2 h-4 w-4" />
                </Button>
                <Button variant="outline" size="lg" className="h-12 px-8 text-base font-semibold" onClick={() => onGo('login')}>
                  Sign In
                </Button>
              </>
            )}
