...
Soubor: `frontend/tests/e2e/app-smoke.spec.ts`

Krome backendovych testu byl doplnen jeden zakladni frontendovy E2E test pomoci Playwrightu. Test nespousti realny backend a nepouziva realnou databazi. API odpovedi jsou mockovane na urovni prohlizece, aby bylo mozne overit zakladni funkcnost frontendove aplikace izolovane.

Test se spousti prikazem:
...
### `renders public landing page and navigates to login`

Test overuje zakladni pruchod verejnou casti aplikace.

Scenar:

1. Playwright spusti Vite dev server.
2. Test zachytava JavaScript chyby z konzole a `pageerror`.
3. Endpoint `/api/auth/session` je namockovan tak, aby aplikace videla neautentizovaneho uzivatele.
4. Ostatni `/api/**` pozadavky jsou namockovane prazdnou JSON odpovedi.
5. Prohlizec otevre hlavni stranku `/`.
6. Test overi, ze je viditelna navigace `TimeCapsule`.
7. Test klikne na tlacitko `Login`.
8. Test overi presmerovani na `/login`.
9. Test overi, ze prihlasovaci formular obsahuje pole `email` a `password`.
10. Na konci se kontroluje, ze pri renderu nevznikla neocekavana JavaScript chyba.
...
- overuje, ze frontend lze spustit v realnem prohlizeci,
- kontroluje zakladni verejnou navigaci,
- zachytava runtime chyby, ktere by unit testy nemusely odhalit,
- predstavuje minimalni E2E vrstvu nad existujicimi backendovymi testy.

Tento test je zamerne maly a stabilni. Nepredstavuje plnohodnotny E2E test celeho systemu od registrace az po ulozeni dat do MongoDB. Takovy test by vyzadoval spusteny backend, testovaci databazi a pripravu testovacich dat. Pro tuto praci ale doplneny smoke test ukazuje, ze projekt ma alespon zakladni automatizovanou kontrolu uzivatelskeho rozhrani v prohlizeci.
